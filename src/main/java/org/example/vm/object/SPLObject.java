package org.example.vm.object;

public class SPLObject {
    public SPLObject add(SPLObject rhs) {
        return null;
    }
    public SPLObject sub(SPLObject rhs) {
        return null;
    }
    public SPLObject mul(SPLObject rhs) {
        return null;
    }
    public SPLObject div(SPLObject rhs) {
        return null;
    }
    public SPLObject mod(SPLObject rhs) {
        return null;
    }
    public SPLObject trueDiv(SPLObject rhs) {
        return null;
    }
    public SPLObject pow(SPLObject rhs) {
        return null;
    }
    public SPLObject lShift(SPLObject rhs) {
        return null;
    }
    public SPLObject rShift(SPLObject rhs) {
        return null;
    }
    public SPLObject URShift(SPLObject rhs) {
        return null;
    }
    public SPLObject and(SPLObject rhs) {
        return null;
    }
    public SPLObject or(SPLObject rhs) {
        return null;
    }
    public SPLObject xor(SPLObject rhs) {
        return null;
    }
    public SPLObject not() {
        return null;
    }
    public SPLObject neg() {
        return null;
    }
    public SPLObject eq(SPLObject rhs) {
        return null;
    }
    public SPLObject ne(SPLObject rhs) {
        return null;
    }
    public SPLObject lt(SPLObject rhs) {
        return null;
    }
    public SPLObject gt(SPLObject rhs) {
        return null;
    }
    public SPLObject le(SPLObject rhs) {
        return null;
    }
    public SPLObject ge(SPLObject rhs) {
        return null;
    }
    public SPLObject conditionalAnd(SPLObject rhs) {
        return null;
    }
    public SPLObject conditionalOr(SPLObject rhs) {
        return null;
    }
    public SPLObject invert() {
        return null;
    }
    public SPLObject call(SPLObject rhs) {
        return null;
    }
    public SPLObject str() {
        return null;
    }
}



